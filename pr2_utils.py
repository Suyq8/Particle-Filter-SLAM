...
def mapCorrelation(im, x_im, y_im, vp, xs, ys):
    ...
    for jy in range(0, nys):
        ...
        tmp = np.zeros(y1.shape)
        iy = np.round((y1-ymin)/yresolution, 0, tmp).astype(np.int16)
        for jx in range(0, nxs):
            x1 = vp[0, :] + xs[jx]  # 1 x 1076
            tmp = np.zeros(x1.shape)
            ix = np.round((x1-xmin)/xresolution, 0, tmp).astype(np.int16)
            valid = (iy >= 0) & (iy < ny) & (ix >= 0) & (ix < nx)

            idx_x = ix[valid]
            idx_y = iy[valid]
            for i in range(len(idx_x)):
                cpr[jx, jy] += im[idx_x[i], idx_y[i]]
    return cpr


def not_mapCorrelation(im, x_im, y_im, vp, xs, ys):
    '''
    INPUT
    im              the map
    x_im,y_im       physical x,y positions of the grid map cells
    ...
    for jy in range(0, nys):
        y1 = vp[1, :] + ys[jy]  # 1 x 1076
        iy = np.round((y1-ymin)/yresolution).astype(np.int16)
        for jx in range(0, nxs):
            x1 = vp[0, :] + xs[jx]  # 1 x 1076
            ix = np.round((x1-xmin)/xresolution).astype(np.int16)
            valid = (iy >= 0) & (iy < ny) & (ix >= 0) & (ix < nx)

            idx_x = ix[valid]
            idx_y = iy[valid]
            cpr[jx, jy] = np.sum(im[idx_x, idx_y])
    return cpr
...
def bresenham2D(sx, sy, ex, ey):
    ...
    if steep:
        ...
        if sx <= ex:
            x = sx*np.ones_like(q) + np.cumsum(q)
        else:
            x = sx*np.ones_like(q) - np.cumsum(q)
    else:
        if sx <= ex:
        ...
        if sy <= ey:
            y = sy*np.ones_like(q) + np.cumsum(q)
        else:
            y = sy*np.ones_like(q) - np.cumsum(q)
